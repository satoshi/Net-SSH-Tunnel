...
sub init {
    ...
    croak "ssh, ps or grep not found"
      unless ( -x $self->{cmds}->{ssh} && -x $self->{cmds}->{ps} && -x $self->{cmds}->{grep} );
}
...
sub check_tunnel {
    ...
    my $command =
"$ps auxw | $grep $ssh | $grep $hostport | $grep $port | $grep $hostname | $grep $host | $grep $user | $grep $sshport | $grep -v grep";
